...
#Currying - Retraso la ejecucion de una funcion
def complete(id):
    def _complete():
...
def render_todos():
    ...
    for i in range(0, len(rows)):
        ...
        #l = Checkbutton(frame, text=description, fg=color, width=42, anchor='w', command=lambda: complete(id))
        l = Checkbutton(frame, text=description, width=42, anchor='w', command=lambda: complete(id))
        l.grid(row=i, column=0, sticky='w')
        btn = Button(frame, text='Eliminar', command=remove(id))
        btn.grid(row=i, column=1)
        #l.select() if completed else l.deselect()

#Funcion que me permite persistir los datos en la BDD
def addTodo():
    todo = e.get()
    if todo:
        c.execute("""
...
#Desde aca en adelante estoy definiendo la ventana en la que voy a hacer
#el Todo
